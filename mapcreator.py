...
class MapCreator:
    ...
    def __init__(self, osmosis_path, pbf_staging_path, map_staging_path, polygons_path,
                  initial_source_pbf, target_path, logging_path, default_start_zoom,
				  default_preferred_language, dry_run=False):
        '''
        Constructor
        '''
        ...
        self.landExtractor = landextraction.LandExtractor(
			self.pbf_staging_path, self.polygons_path, self.dry_run)
    ...
    def evalPart(self, subtree, source_pbf, staging_path, target_dir,
		zoom_interval_conf, land_simplification):
        ...
        for child in subtree:
			...
			# we do not need to filter the area during map creation if either a
			# pbf was created (with a filter) or the source pbf equals the
			# current part
            ...
            if create_pbf:
                # create pbf
                try:
                    self.logger.debug("staging_path=%s", staging_path)
					...
					# print "an error occurred while creating pbf for '%s', no
					# further processing of sub-parts will take part, error
					# message: %s" % (current_part_name, e)
            ...
            if create_map:
                ...
                self.landExtractor.extract_land_polygons(staging_path + current_part_name,
					self.pbf_staging_path, land_simplification, self.dry_run)
                try:
                    self.call_create_map(new_source_pbf, staging_path, target_dir,
						current_part_name, area_filter, map_start_zoom, preferred_language,
						zoom_interval_conf, storage_type, map_start_lat, map_start_lon)
                except ProcessingException, e:
                    error_occurred = True
                    self.logger.warning("%s", str(e))
					# print "an error occurred while creating map for '%s',
					# error message: %s" % (current_part_name, e)
            ...
            #### RECURSION
            subpart_error = self.evalPart(child, new_source_pbf, new_staging_path,
				new_target_dir, zoom_interval_conf, land_simplification)
    ...
    def call_create_pbf(self, source_pbf, staging_dir, current_part_name):
        ...
        if not self.dry_run:
            if not PATH.exists(source_pbf_path):
                raise ProcessingException('cannot create %s, source pbf is missing: %s' % (
					target_pbf,source_pbf_path))
            if PATH.getsize(source_pbf_path) == 0:
                raise ProcessingException('cannot create %s, source pbf is empty: %s' % (
					target_pbf,source_pbf_path))

        polygons_path = self.polygons_path + staging_dir + current_part_name+'.poly'
        if not PATH.exists(polygons_path):
            raise ProcessingException('cannot create pbf %s , polygon is missing: %s' % (
				target_pbf, polygons_path))
        ...
        try:
            self.logger.debug("`%s`"," ".join(osmosis_call))
            if not self.dry_run:
            ...
            logfile.close()
            raise ProcessingException("call to osmosis raised an error, see logs at %s for further details"%
				logfile_path)
        except OSError,e:
            raise ProcessingException("osmosis executable not found: %s"%e)

    def call_create_map(self, source_pbf, staging_dir, target_dir, current_part_name, area_filter,
		start_zoom,preferred_language, zoom_interval_conf, storage_type='ram', lat=None,lon=None):
        ...
        if not self.dry_run:
            if not PATH.exists(source_pbf_path):
                raise ProcessingException('cannot create map %s, source pbf is missing: %s' % (
					map_file,source_pbf_path))
            if PATH.getsize(source_pbf_path) == 0:
                raise ProcessingException('cannot create map %s, source pbf is empty: %s' % (
					map_file,source_pbf_path))
        ...
        if area_filter:
            ...
            if not PATH.exists(polygon_file_path):
                raise ProcessingException('cannot create map %s, polygon is missing: %s' % (
					map_file, polygon_file_path))
        ...
        if not self.dry_run and PATH.getsize(map_file_path) == 0:
            raise ProcessingException("resulting map file size for %s is zero, keeping old map file"%
				map_file)
        ...
        self.logger.debug("`%s`"," ".join(move_call))
        if not self.dry_run:
            try:
