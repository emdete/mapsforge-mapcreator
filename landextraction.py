...
class LandExtractor:
    ...
    def __init__(self, output_dir, polygon_dir, dry_run = False):
        self.logger = logging.getLogger("landextraction")
    ...
    def download_land_polygons(self, data_dir):
        ...
        url = "http://data.openstreetmapdata.com/" + self.landfiles + ".zip"
        self.logger.debug("`wget %s -o %s`", url, path)
        if not self.dry_run:
            urllib.urlretrieve (url, path)
        self.logger.debug("#done")
        self.logger.debug("`cd %s; unzip ../%s`", data_dir, path)
        if not self.dry_run:
            zipfile.ZipFile(path).extractall(data_dir)
        self.logger.debug("#done")

    def extract_land_polygons(self, region, data_dir, simplify=0, dry_run=False):
        import subprocess
        simplifiy = 0.2
        bbox = self.region_bbox(region)
        if simplify == 0:
            ogr_call = ["ogr2ogr",
                "-overwrite",
                "-skipfailures",
                "-clipsrc", str(bbox[0]), str(bbox[1]), str(bbox[2]), str(bbox[3]),
                os.path.join(self.output_dir, region.replace("/", "-")),
                os.path.join(data_dir, os.path.join(self.landfiles, "land_polygons.shp"))]
        else:
            ogr_call = ["ogr2ogr",
                "-overwrite",
                "-skipfailures",
                ...
                "-clipsrc", str(bbox[0]), str(bbox[1]), str(bbox[2]), str(bbox[3]),
                os.path.join(self.output_dir, region.replace("/", "-")),
                os.path.join(data_dir, "land-polygons-split-4326/land_polygons.shp")]
        self.logger.debug("`%s`"," ".join(ogr_call))
        if not dry_run:
            success = subprocess.call(ogr_call)
        self.logger.info("#done")
        self.logger.info("#shape2osm")
        if not dry_run:
